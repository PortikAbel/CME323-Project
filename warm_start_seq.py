...
def find_maximum_matching(G,M):
    P = finding_aug_path(G,M,Forest=[],tree_to_root={}, Forest_nodes=[])
    if P == []:#Base Case
        ...
        for i in range(0,len(P)-2,2): #Parallel For
...
def finding_aug_path(G,M,Forest=[], tree_to_root={}, Forest_nodes=[], Blossom_stack=[]):
    ...
    for v in Forest_nodes:  ## <----------- we cant trivially change this...
        ...
        root_of_v = None
        tree_num_of_v = None
        for tree_number in range(len(Forest)): ## <------- completely parallel : figuring out the tree of v
        ...
        for edge_number in range(len(edges_v)): ## <--------  we can DO this!!! NON Triviall
                ...
                if w_in_Forest == 0:
                    ...
                    if dist_to_root(w,root_of_w,Forest[tree_num_of_w])%2 == 0:
                        ...
                        #w = e[0][1] # the other vertex of the unmarked edge
                            ...
                            contracted_F = copy.deepcopy(Forest)
                            # print("w is: ", w)
                            for node in blossom[0:len(blossom)-1]:
                                ...
                                if node != w:
                                    ...
                                    contracted_F[tree_num_of_v] = nx.contracted_nodes(contracted_F[tree_num_of_v], w, node, self_loops=False)
                            ...
                            contracted_t2r = copy.deepcopy(tree_to_root)
                            if root_of_v in blossom:
                                contracted_t2r[tree_num_of_v] = w
                            ...
                            # recurse
                            v_idx = Forest_nodes.index(v)
                            nodes_to_check = list(set(Forest_nodes[v_idx:]) - set(blossom))
                            aug_path = finding_aug_path(contracted_G, contracted_M, contracted_F, contracted_t2r, nodes_to_check, Blossom_stack)
                            # print("\t \t ~~~")
